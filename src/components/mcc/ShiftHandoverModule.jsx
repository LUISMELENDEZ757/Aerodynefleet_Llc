    ...
    <div className="fixed inset-0 z-50 bg-black/70 p-4 flex justify-center items-start overflow-y-auto pt-8">
      <div className="w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 z-10 bg-card">
          <p className="text-base font-bold text-foreground">Shift Handover Log</p>
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-1">Your Name</label>
              <input
                type="text"
                value={form.submitted_by}
                onChange={e => set('submitted_by', e.target.value)}
                placeholder="Full name"
                className="w-full bg-secondary border border-border rounded-xl px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-primary"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={form.submitted_by_cert}
              onChange={e => set('submitted_by_cert', e.target.value)}
          ...
          <div>
            <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-1">Progress Summary *</label>
            <textarea
              rows={3}
              value={form.progress_summary}
              onChange={e => set('progress_summary', e.target.value)}
              placeholder="Summarize work completed this shift..."
              className="w-full bg-secondary border border-border rounded-xl px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-primary resize-none"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              rows={2}
              value={form.safety_critical_notes}
              onChange={e => set('safety_critical_notes', e.target.value)}
              placeholder="Any safety hazards, hazmat, or critical conditions..."
              className="w-full bg-red-950/20 border border-red-500/30 rounded-xl px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-red-400 resize-none"
            />
          </div>

          <div>
            <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-2">Pending Issues</label>
                ...
                <div key={idx} className="bg-secondary rounded-xl p-3 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      {issue.aircraft_tail && <span className="text-xs font-mono font-bold text-primary">{issue.aircraft_tail}</span>}
            ...
            <div className="bg-secondary rounded-xl p-3 space-y-2">
              <input
                ...
                className="w-full bg-background border border-border rounded-lg px-2 py-2 text-sm text-foreground outline-none focus:border-primary"
              />
              <div className="grid grid-cols-3 gap-2">
                <input
                  type="text"
                  value={newIssue.aircraft_tail}
                  onChange={e => setNewIssue(p => ({ ...p, aircraft_tail: e.target.value }))}
                  placeholder="Tail (optional)"
                  className="bg-background border border-border rounded-lg px-2 py-2 text-xs text-foreground outline-none focus:border-primary"
                />
                ...
                <input
                  ...
                  onChange={e => setNewIssue(p => ({ ...p, assigned_to: e.target.value }))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-1">Parts Consumed</label>
              <input
                type="text"
                value={form.parts_consumed}
                onChange={e => set('parts_consumed', e.target.value)}
                placeholder="e.g. 2x O-rings, 1x valve"
                className="w-full bg-secondary border border-border rounded-xl px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-primary"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest block mb-1">Tools In Use</label>
              <input
                type="text"
                value={form.tools_in_use}
                onChange={e => set('tools_in_use', e.target.value)}
                placeholder="e.g. TRQ-001, Lift #3"
                className="w-full bg-secondary border border-border rounded-xl px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-primary"
              />
            </div>
          </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-border">
          ...
          <button onClick={handleSubmit} disabled={isPending || !form.submitted_by || !form.progress_summary} className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center gap-2">
            <Send className="w-4 h-4" /> {isPending ? 'Submitting...' : 'Submit Handover'}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
      ...
      {criticalIssues.length > 0 && (
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-bold text-orange-400">{criticalIssues.length} critical issue{criticalIssues.length !== 1 ? 's' : ''}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold text-foreground">Shift Handover Log</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="flex items-center gap-2 bg-secondary rounded-xl px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground">No shift handovers yet</p>
        </div>
      ) : (

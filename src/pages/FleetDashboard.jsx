...
import {
  ...
  ChevronDown, Radio, Activity, Package, Brain, Settings2
} from 'lucide-react';
...
function AircraftCard({ aircraft, onSelect, discrepancies, maintenanceStatus }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-gray-500">Base: <span className="font-bold text-gray-300">{aircraft.base_station || '—'}</span></p>
...
function AircraftDetailOverlay({ aircraft: initialAircraft, onClose }) {
  ...
  const statusCfg = STATUS_STYLES[aircraft.status] || STATUS_STYLES.active;
  ...
  const catBorder = isDowngraded ? 'border-red-600/40' : isCatI ? 'border-yellow-600/40' : 'border-green-600/40';
  const catBg = isDowngraded ? 'bg-red-900/20' : isCatI ? 'bg-yellow-900/20' : 'bg-green-900/20';
  const catText = isDowngraded ? 'text-red-400' : isCatI ? 'text-yellow-400' : 'text-green-400';

  const isNonEtops = etopsStatus === 'NON-ETOPS';
  const isUltraEtops = etopsStatus === 'ETOPS-370' || etopsStatus === 'ETOPS-330';
  const etopsBorder = isNonEtops ? 'border-red-600/40' : isUltraEtops ? 'border-cyan-600/40' : 'border-green-600/40';
  const etopsBg = isNonEtops ? 'bg-red-900/20' : isUltraEtops ? 'bg-cyan-900/20' : 'bg-green-900/20';
  const etopsText = isNonEtops ? 'text-red-400' : isUltraEtops ? 'text-cyan-400' : 'text-green-400';
    ...
    >
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#141922] sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <span className={cn('flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white ml-2', statusCfg.bg)}>
      ...
      <div className="flex flex-col lg:flex-row gap-0 min-h-[calc(100vh-65px)]">
        ...
        <div className="w-full lg:w-72 flex-shrink-0 bg-[#111620] border-r border-white/10 p-6 flex flex-col gap-5">
          ...
          {/* CAT Selector */}
          <div className={cn('rounded-xl border px-4 py-3 relative', catBorder, catBg)}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Shield className={cn('w-4 h-4', catText)} />
                <p className={cn('text-[10px] font-bold uppercase tracking-widest', catText)}>ILS / CAT STATUS</p>
              </div>
            ...
            <p className={cn('text-[10px]', catText)}>Max certified: <span className="font-bold">{catOptions[0]}</span></p>
            {showCatDropdown && (
              <div className="absolute right-3 top-full mt-1 bg-[#1a1f2e] border border-white/10 rounded-xl overflow-hidden z-20 shadow-xl min-w-[180px]">
                {catOptions.map(opt => (
                  <button key={opt} onClick={() => { setCatStatus(opt); setShowCatDropdown(false); }}
                    className={cn('block w-full text-left px-4 py-2.5 text-xs font-bold hover:bg-white/10',
                      opt === 'DOWNGRADED' ? 'text-red-400' : opt === 'CAT I' ? 'text-yellow-400' : 'text-green-300'
                    )}>{opt}</button>
                ))}
              </div>
            )}
          ...
          {/* ETOPS Selector */}
          <div className={cn('rounded-xl border px-4 py-3 relative', etopsBorder, etopsBg)}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Globe className={cn('w-4 h-4', etopsText)} />
                <p className={cn('text-[10px] font-bold uppercase tracking-widest', etopsText)}>ETOPS STATUS</p>
              </div>
            ...
            <p className={cn('text-[10px]', etopsText)}>Max certified: <span className="font-bold">{etopsOptions[0]}</span></p>
            {showEtopsDropdown && (
              <div className="absolute right-3 top-full mt-1 bg-[#1a1f2e] border border-white/10 rounded-xl overflow-hidden z-20 shadow-xl min-w-[160px]">
                {etopsOptions.map(opt => (
                  <button key={opt} onClick={() => { setEtopsStatus(opt); setShowEtopsDropdown(false); }}
                    className={cn('block w-full text-left px-4 py-2.5 text-xs font-bold hover:bg-white/10',
                      opt === 'NON-ETOPS' ? 'text-red-400' : opt === 'ETOPS-370' ? 'text-cyan-300' : 'text-green-300'
                    )}>{opt}</button>
                ))}
              </div>
            )}
...
export default function FleetDashboard() {
  ...
  const getMaintenanceStatus = (tail) => {
    if (oosAircraft.has(tail)) return 'oos';
    if (melByTail.has(tail)) return 'mel';
    return 'airworthy';
  };
  ...
  const filtered = aircraft.filter(a => {
    ...
    const matchesKpi = !kpiFilter || getMaintenanceStatus(a.tail_number) === kpiFilter;
    return matchesSearch && matchesStatus && matchesKpi;
  });

  const acTypeLabel = activeFleet ? `${activeFleet.name} Aircraft` : 'Boeing 737 Aircraft';
    ...
    <div className="min-h-screen bg-background pb-24">
      <div className="px-6 pt-7 pb-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <KpiCard label="Total Fleet" value={isLoading ? '…' : total.toLocaleString()} sublabel={acTypeLabel} valueColor="text-white" icon={Plane} iconColor="text-primary" onClick={() => setKpiFilter(null)} />
          <KpiCard label="Airworthy" value={isLoading ? '…' : airworthy.toLocaleString()} sublabel="No Restrictions" valueColor="text-green-400" icon={CheckCircle} iconColor="text-green-400" onClick={() => setKpiFilter(kpiFilter === 'airworthy' ? null : 'airworthy')} />
          <KpiCard label="Airworthy with MEL" value={isLoading ? '…' : withMel.toLocaleString()} sublabel="Dispatch Restricted" valueColor="text-yellow-400" icon={AlertTriangle} iconColor="text-yellow-400" onClick={() => setKpiFilter(kpiFilter === 'mel' ? null : 'mel')} />
          <KpiCard label="Out of Service" value={isLoading ? '…' : outOfSvc.toLocaleString()} sublabel="Awaiting Service" valueColor={outOfSvc > 0 ? 'text-orange-400' : 'text-gray-600'} icon={Clock} iconColor={outOfSvc > 0 ? 'text-orange-400' : 'text-gray-600'} onClick={() => setKpiFilter(kpiFilter === 'oos' ? null : 'oos')} />
        </div>
      </div>
      ...
      {activeTab === 'fleet' && (
        <div className="px-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {kpiFilter && (
              <button onClick={() => setKpiFilter(null)} className="text-xs font-bold text-primary hover:text-primary/80 flex items-center gap-1">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
              {filtered.map(a => (
                <AircraftCard key={a.id} aircraft={a} onSelect={setSelectedAircraft} discrepancies={discrepanciesByTail[a.tail_number]} maintenanceStatus={getMaintenanceStatus(a.tail_number)} />
              ))}
            </div>
          ) : (
            <div className="flex flex-col gap-1.5">
              {filtered.map(a => (
                <AircraftRow key={a.id} aircraft={a} onSelect={setSelectedAircraft} discrepancies={discrepanciesByTail[a.tail_number]} maintenanceStatus={getMaintenanceStatus(a.tail_number)} />
              ))}
            </div>
          )}
        </div>
      ...
      {activeTab === 'rob' && (
        <div className="px-6 flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-gray-600 text-sm text-center max-w-sm">Robbed parts tracking and bill of resources coming soon.</p>
      ...
      {activeTab === 'mxcomms' && (
        <div className="px-6 flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-gray-600 text-sm text-center max-w-sm">Live maintenance communications feed.</p>

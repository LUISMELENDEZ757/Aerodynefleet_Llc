...
export default function ManpowerStaffing() {
  ...
  const { data: technicians = [], isLoading, refetch } = useQuery({
    ...
    select: (data) => data.map(r => ({
      ...
      employee_id: r.employee_id || r.certificate_number || '',
      station: r.station || r.base_station || 'KEWR',
      years_experience: r.years_experience || 0,
      certifications: r.certifications || r.certificates || [],
      specialty: r.specialty || r.role || '',
      status: r.duty_status || r.status || 'available',
      current_assignment: r.current_aircraft || r.aircraft_tail || '',

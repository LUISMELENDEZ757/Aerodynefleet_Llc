...
import {
  ...
  Download, Clipboard, Eye, Shield, Activity, Filter,
  Clock, Zap, Search, MapPin
} from 'lucide-react';
import { cn } from '@/lib/utils';
import FullWorkflowPipeline from '@/components/qa/FullWorkflowPipeline';
import QCPartsInspectionTab from '@/components/qa/QCPartsInspectionTab';
...
function KpiCard({ icon: Icon, label, value, dotColor }) {
  return (
...
function TopNav({ activeTopTab, setActiveTopTab }) {
  return (
...
function QCInspectionView({ selectedStation, setSelectedStation, selectedInspectionType, setSelectedInspectionType, setActiveTopTab, activeTopTab }) {
  const [subTab, setSubTab] = useState('parts_borrob');

  const SUB_TABS = [
    { id: 'parts_borrob', label: '📦 BOR/ROB Parts Inspection' },
    { id: 'standard_qc',  label: '🔍 Standard QC Inspections' },
  ];
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-90">Quality Control Inspection Management • RII Tracking • BOR/ROB Parts Approval</p>
            </div>
          </div>
          <LiveClock />
        </div>
      ...
      {/* Sub-tab navigation */}
      <div className="border-b border-border bg-secondary/30 px-5 py-3 flex gap-2">
        {SUB_TABS.map(({ id, label }) => (
          <button key={id} onClick={() => setSubTab(id)}
            className={cn(
              'px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all',
              subTab === id ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
            )}>
            {label}
          </button>
        ))}
      ...
      {subTab === 'parts_borrob' && (
        <div className="p-5 max-w-6xl mx-auto">
          <div className="mb-5">
            <h2 className="text-xl font-extrabold text-foreground">BOR/ROB Parts Inspection Queue</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Inspect and approve or reject robbed/borrowed parts before installation</p>
          </div>
          <QCPartsInspectionTab />
        </div>
      )}
      ...
      {subTab === 'standard_qc' && (
        <div className="flex">
          {/* Sidebar */}
          <div className="w-72 border-r border-border bg-secondary/20 p-4 flex flex-col gap-4 min-h-[calc(100vh-320px)]">
            ...
            <div className="space-y-3">
              ...
              <select value={selectedStation} onChange={e => setSelectedStation(e.target.value)} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-xs outline-none text-foreground">
            ...
            <div className="space-y-2">
              ...
              {[
                ...
                <button key={id} onClick={() => setSelectedInspectionType(id)}
                  className={cn('w-full text-left p-3 rounded-xl border transition-all',
                    selectedInspectionType === id ? 'bg-yellow-600/30 border-yellow-500/50' : 'border-border hover:border-border/80'
                  )}>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={cn('text-xs font-extrabold px-1.5 py-0.5 rounded-full',
                      selectedInspectionType === id ? 'bg-yellow-500/30 text-yellow-600' : 'bg-muted text-muted-foreground'
                    )}>{count}</span>
                  </div>
                </button>
              ))}
            ...
            {[
              ...
              <div key={i} className="bg-card border border-border rounded-2xl p-5 space-y-4">
                ...
                <div className="grid grid-cols-4 gap-4 pt-2 border-t border-border text-xs">
                  <div><p className="text-muted-foreground">Aircraft</p><p className="font-bold">{item.aircraft}</p></div>
                  <div><p className="text-muted-foreground">Work Order</p><p className="font-bold text-blue-400">{item.workOrder}</p></div>
                  <div><p className="text-muted-foreground">ATA Chapter</p><p className="font-bold">{item.ata}</p></div>
                  <div><p className="text-muted-foreground">Station</p><p className="font-bold flex items-center gap-1"><MapPin className="w-3 h-3" />{item.station}</p></div>
                </div>
                <div className="flex items-center justify-between pt-2 border-t border-border text-xs">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="flex items-center gap-1"><Users className="w-3 h-3 text-muted-foreground" />{item.inspector}</span>
                  ...
                  <button className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 text-white font-bold px-3 py-1.5 rounded-lg transition-colors">
...
function QAEnvView({ selectedModule, setSelectedModule, setActiveTopTab, activeTopTab }) {
  return (
    <div className="min-h-screen bg-background pb-24">
      <TopNav activeTopTab={activeTopTab} setActiveTopTab={setActiveTopTab} />
      ...
      <div className="flex">
        <div className="w-64 border-r border-border bg-secondary/20 p-4 flex flex-col gap-4 min-h-[calc(100vh-300px)]">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          ...
          <div className="space-y-2">
            ...
            {MODULES.map(({ id, label, icon: Icon, desc }) => (
              <button key={id} onClick={() => setSelectedModule(id)}
                className={cn('w-full text-left p-3 rounded-xl border transition-all',
                  selectedModule === id ? 'bg-yellow-600/30 border-yellow-500/50 text-yellow-600' : 'border-border hover:border-border/80'
                )}>
          ...
          {selectedModule === 'fleet_health' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button className="flex items-center gap-1.5 bg-secondary text-muted-foreground hover:text-foreground text-xs font-bold px-3 py-2 rounded-lg">
                    <Filter className="w-3.5 h-3.5" /> Filter
                  </button>
              ...
              <div className="bg-card/50 border border-border rounded-2xl p-6">
                <h3 className="text-lg font-extrabold text-foreground mb-2">📊 FLEET HEALTH & COMPLIANCE OVERVIEW</h3>
                ...
                <div className="flex gap-2 flex-wrap">
                  {[
                    ...
                    <button key={label} className={cn('px-4 py-2 rounded-lg text-xs font-bold', color)}>{icon} {label}</button>
                  ))}
                </div>
              </div>
            </div>
...
function DashboardView({ activeTab, setActiveTab, setActiveTopTab, activeTopTab }) {
  return (
    <div className="min-h-screen bg-background pb-24">
      <TopNav activeTopTab={activeTopTab} setActiveTopTab={setActiveTopTab} />
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
          {TABS.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              className={cn('flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-extrabold whitespace-nowrap transition-all flex-shrink-0',
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard icon={Clipboard}     label="Total RII Items"    value="5" />
              <KpiCard icon={TrendingUp}    label="RII Pass Rate"      value="50%" />
              <KpiCard icon={AlertTriangle} label="Open Findings"      value="2" />
              <KpiCard icon={Users}         label="Active Inspectors"  value="3" />
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-card border border-border rounded-2xl p-5">
                <p className="text-sm font-extrabold text-foreground mb-4">Findings by Severity</p>
                <div className="space-y-3">
                  {[{ label: 'Critical', count: 1, color: 'bg-red-600' }, { label: 'Major', count: 2, color: 'bg-orange-500' }, { label: 'Minor', count: 2, color: 'bg-yellow-500' }].map(({ label, count, color }) => (
                    <div key={label} className="flex items-center gap-3">
                      <span className="text-xs text-muted-foreground w-12">{label}</span>
                      <div className="flex-1 bg-secondary rounded-full h-2"><div className={cn('h-full rounded-full', color)} style={{ width: `${(count / 2) * 100}%` }} /></div>
              ...
              <div className="bg-card border border-border rounded-2xl p-5">
                ...
                <div className="space-y-2">
                  {[{ label: 'Pending Assignment', count: 2 }, { label: 'Assigned', count: 1 }, { label: 'Completed Pass', count: 1 }].map(({ label, count }) => (
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={name} className="flex items-center justify-between p-3 bg-background/40 rounded-lg">
                    ...
                    <span className={cn('text-xs font-bold px-2.5 py-1 rounded-full text-white', statusColor)}>{status}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              {[
                ...
                <div key={i} className="px-5 py-3">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div className="flex-1">
                      <p className="text-sm font-bold text-foreground">{f.finding}</p>
                      <p className="text-xs text-muted-foreground">ATA {f.ata}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className={cn('text-xs font-bold px-2 py-1 rounded',
                        f.severity === 'Critical' ? 'bg-red-500/20 text-red-600' : f.severity === 'Major' ? 'bg-orange-500/20 text-orange-600' : 'bg-yellow-500/20 text-yellow-600'
                      )}>{f.severity}</span>
                      <span className={cn('text-xs font-bold px-2 py-1 rounded',
                        f.status === 'Open' ? 'bg-red-500/20 text-red-600' : 'bg-green-500/20 text-green-600'
                      )}>{f.status}</span>
                    </div>
                  </div>
            ...
            {[
              ...
              <div key={id} className="bg-card border border-border rounded-2xl p-5 space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <span className={cn('text-xs font-bold px-2 py-1 rounded-full text-white', status === 'active' ? 'bg-green-600' : 'bg-red-600')}>{status}</span>
                </div>
...
export default function QAQCDashboard() {
  ...
  if (activeTopTab === 'qc_inspection') {
    return <QCInspectionView
      ...
      selectedInspectionType={selectedInspectionType} setSelectedInspectionType={setSelectedInspectionType}
      activeTopTab={activeTopTab} setActiveTopTab={setActiveTopTab}
    />;
  }
